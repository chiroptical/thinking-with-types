...
type family RequireUniqueKey
  ...
  RequireUniqueKey 'False key t ts =
    TypeError
      (     'Text "Attempting to insert with key named `"
      ':<>: 'Text key
      ':<>: 'Text "' and value of type `"
      ':<>: 'ShowType t
      ':<>: 'Text "' to an OpenProduct."
      ':$$: 'Text "But, the OpenProduct already has a key `"
      ':<>: 'Text key
      ':<>: 'Text "' with value of type `"
      ':<>: 'ShowType (LookupType key ts)
      ':<>: 'Text "'."
      ':$$: 'Text "Consider using `update' or `upsert'"
